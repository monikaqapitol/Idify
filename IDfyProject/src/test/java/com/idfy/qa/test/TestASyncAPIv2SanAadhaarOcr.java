...
public class TestASyncAPIv2SanAadhaarOcr {
	...
	public void testAsyncAadharOcr(APIPostAsyncRequests request, APIPostAsyncVoterOcrResponse expectedResponse)
		...
		postResponse = asyncCalls.aadharPOSTOcr(request);
		Thread.sleep(9000);
		response = asyncCalls.aadharGETocr(postResponse.getRequestId());
		Thread.sleep(5000);
		...
		try {
			if(response[0].getOcrOutput().getAadhaarNumber()!=null) {
				exList.add("aadhaar_number");
			}}catch(NullPointerException e) {
		...
		try {
			if(response[0].getOcrOutput().getYearOfBirth()!=null) {
				exList.add("year_of_birth");
			}}catch(NullPointerException e) {
		...
		try {
			if(response[0].getOcrOutput().getDistrict()!=null) {
				exList.add("district");
			}}catch(NullPointerException e) {
		...
		try {
			if(response[0].getError().getMessage()!=null) {
				exList.add("message");
			}}catch(NullPointerException e) {
	...
	public void testVoterOcrAadhaarNumber()
			throws IOException, InterruptedException{
			Assert.assertTrue(exList.contains("aadhaar_number"),makeAttachFalse("aadhaar_number"));
			makeAttachTrue("aadhaar_number");
	}
	...
	@Test(description = "Assertion for year_of_birth",priority=14)
	public void testVoterOcrAge()
			throws IOException, InterruptedException{
			Assert.assertTrue(exList.contains("year_of_birth"),makeAttachFalse("year_of_birth"));
			makeAttachTrue("year_of_birth");
	}
	@Test(description = "Assertion for gender",priority=15)
	public void testVoterOcrGender()
			throws IOException, InterruptedException{
			Assert.assertTrue(exList.contains("gender"),makeAttachFalse("gender"));
			makeAttachTrue("gender");
	}
	@Test(description = "Assertion for fathers_name",priority=16)
